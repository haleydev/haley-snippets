...
const ViewFormattingEditProvider_1 = require("./providers/ViewFormattingEditProvider");
...
function activate(context) {
    ...
    let viewFormatCfg = vscode.workspace.getConfiguration('view.format');
    if (viewFormatCfg.get('enable')) {
        context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider(documentSelector, new ViewFormattingEditProvider_1.ViewFormattingEditProvider));
        context.subscriptions.push(vscode.languages.registerDocumentRangeFormattingEditProvider(documentSelector, new ViewFormattingEditProvider_1.ViewFormattingEditProvider));
    }
    // Set html indent
    const EMPTY_ELEMENTS = ['area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'menuitem', 'meta', 'param', 'source', 'track', 'wbr'];
    vscode.languages.setLanguageConfiguration('view', {
        indentationRules: {
    ...
    let clientOptions = {
        documentSelector: ['view'],
        synchronize: {
            configurationSection: ['view', 'css', 'javascript', 'emmet'], // the settings to synchronize
        },
